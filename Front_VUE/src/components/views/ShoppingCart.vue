<template>
  <div>
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item style="margin-left:20px;margin-top:20px">shopping cart</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="goodslist" border stripe v-on:selection-change="handleSelectionChange">
        ...
        <el-table-column label="Commodity name" prop="productName"  ></el-table-column>
        <el-table-column
      ...
      <i class="el-icon-remove" @click="down(scope.row)"></i>
     {{scope.row.quantity}}
     <i class="el-icon-circle-plus" @click="up(scope.row)" ></i>
   </template></el-table-column>
           <el-table-column
          label="Total price (RMB)"
          prop="totalPrice"
          width="95px"
        ></el-table-column>
      ...
      <div class="cart-filter-bar-bottom">
        <div> <el-button type="primary" icon="el-icon-arrow-left" round style=" margin-top: 6%" @click="goback()">go back</el-button></div>
            ...
            <div class="cart-sum">
              ...
              <div v-if="selectionData.length > 0"  class="pay-btn-active" @click="addToOrder()">
                submit
              </div>
              <div v-else class="pay-btn-inactive" >submit</div>
            </div>
      </div>
    ...
    >
      ...
      >
            ...
            <el-form-item label-width="120px" label="receiverName" prop="receiverName">
              <el-input v-model="addForm.receiverName"></el-input>
            </el-form-item>
            <el-form-item label-width="120px" label="receiverMobile" prop="receiverMobile">
              <el-input v-model="addForm.receiverMobile" type="number"></el-input>
            </el-form-item>
            <el-form-item label-width="120px" label="receiverAddress" prop="receiverAddress">
              <el-input v-model="addForm.receiverAddress" ></el-input>
            </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="addCateDialogVisible = false">Cancel</el-button>
        <el-button type="primary" @click="addOrders2()">OK</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      title="Please pay"
      :visible.sync="PaddCateDialogVisible"
      width="50%"
      @close="PaddCateDialogClosed()"
    >
      <!-- 主体区 -->
      <img src="../../assets/code.png" alt="">
      <span slot="footer" class="dialog-footer">
        <el-button @click="PaddCateDialogVisible = false">Cancel</el-button>
        <el-button type="primary" @click="addOrders()">OK</el-button>
      </span>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    addOrders2(){
      ...
      this.PaddCateDialogVisible=true},
    addOrders(){
    
        this.$http.post("/order/create", this.addForm).then((res) => {
          if (res.data.status !== 10000) {
            return this.$message.error("Failed to submit order");
          }
          this.order_No = res.data.data;
          this.$message.success("The order was submitted successfully");
          this.getGoodList();
         
          this.PaddCateDialogVisible = false
    ...
    down(element) {
          if (element.quantity<=0) {
            return this.$message.error("Reduce merchandise failures");
            

          }
          this.$http
          .post("/cart/update" , {productId:element.productId,count:element.quantity-1})
          .then((res) => {
            if (res.data.status !== 10000) {
              return this.$message.error("Reduce merchandise failures");
            }
            this.$message.success("Reduce merchandise success");
            this.getGoodList();
            })
    },
    removeById(element) {
      ...
      this.$confirm("This action will remove the item from the shopping cart. Continue or not?", "tip", {
        ...
        .then((res) => {
          ...
          this.$http.post("/cart/delete" ,{productId:element.productId}).then((res) => {
            if (res.data.status !== 10000) {
              return this.$message.error("Failed to delete product");
            }
            this.$message.success("Removed item successfully");
            this.getGoodList();
          });
        })
        .catch((err) => {
          ...
          return this.$message.info("Undeleted");
        });
    },
    up(element) {
        this.$http
          ...
          .then((res) => {
            if (res.data.status !== 10000) {
              return this.$message.error("Failed to add goods number");
            }
            this.$message.success("Increase commodity success");
            this.getGoodList();
            })
    }
